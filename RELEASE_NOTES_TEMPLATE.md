For an installation guide, refer to the [README](https://github.com/hubertusgbecker/mcp-linkedin-server/blob/main/README.md).

## 🐳 Update Docker Installation
**For users with Docker-based MCP client configurations:**
```bash
docker pull hubertusgbecker/mcp-linkedin-server:latest
```
The `latest` tag will always point to the most recent release.
To pull this specific version, run:
```bash
docker pull hubertusgbecker/mcp-linkedin-server:${VERSION}
```
